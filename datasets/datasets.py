    ...
    def __getitem__(self, i):
        ...
        if self.segmentation:
            ...
            for i in range(self.num_segs):
                seg_samples[i] = self.get_segs(sample)
...
class ShapeNetAllDataset(Dataset):
    def __init__(self, path2data, part='train',
        ...
        self.catfile = './synsetoffset2category.txt'
        self.num_seg_classes_file = './num_seg_classes.txt'

    def get_num_segs(self, label):
        cat = {}
        ...
        with open(self.catfile, 'r') as f:
            for line in f:
                ls = line.strip().split()
                if label == ls[1]:          #假设两边label都是数字
                    cat = ls[0]             #获取类别

        with open(self.num_seg_classes_file, 'r') as f:
            for line in f:
                ls = line.strip().split()
                if cat == ls[0]:
                    num_segs = ls[1]        #获取类别对应的seg数量
    ...
    def __getitem__(self, i):
        ...
        if self.sample_labels:
            ...
            sample['num_segs'] = self.get_num_segs(sample['label'])
